...
from nesyrl.agents.physical import DualActorProb
...
log_dir = f"results/physical/move/a2c_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
...
if __name__ == "__main__":
    ...
    for run in range(args["num_runs"]):
        ...
        # Prepare agent
        actors = []
        agent_state_shape = (train_env.observation_space.shape[0] - 1,)
        for _ in range(2):
            net = Net(state_shape=agent_state_shape,
        ...
        for a in (actor.actor0, actor.actor1):
            a.mu.apply(lambda m: rescale_weights(m, args["actor"]["mu_scale"]))
            torch.nn.init.constant_(a.sigma_param, args["actor"]["sigma_param"])
