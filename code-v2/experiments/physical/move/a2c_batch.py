...
log_dir = f"results/physical/move/a2c_batch_{int(time.time())}"
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        train_env = gym.make("nesyrl-physical/NicoBlocksWorldMove-v0", **env_kwargs)
        test_env = gym.make("nesyrl-physical/NicoBlocksWorldMove-v0", render_mode=None, **env_kwargs)
        ...
        # Prepare agent
        # TODO: wider value, narrower policy
        ...
        actor.mu.apply(lambda m: rescale_weights(m, 0.01))
        torch.nn.init.constant_(actor.sigma_param, -4.5)
        ...
        # Prepare training
        train_collector = Collector(policy, train_venv)
        ...
        trainer = OnpolicyTrainer(
            ...
            batch_size=64,
            logger=logger,
            test_fn=lambda _, __: test_venv.set_obs_rms(train_venv.get_obs_rms())
        )
        ...
        # Save models
        torch.save(net, f"{log_dir}/run_{run}_net.model")
        ...
        save_obs_norm_params(train_venv.get_obs_rms(), f"{log_dir}/run_{run}.norm")
