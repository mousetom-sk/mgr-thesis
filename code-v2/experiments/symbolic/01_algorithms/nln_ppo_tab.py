...
from nesyrl.logic.neural import ConstantInitializer, UniformInitializer, NLAndProd, NLAndLuka, NLAndBiProd, NLAndBiLuka, CorrectionPostprocessor, NoPostprocessor
...
parser.add_argument("--actor-negated", help="if the actor should have negations as extra inputs", action="store_true")
parser.add_argument("--actor-assume-false", help="if the actor should set a high weight to false initially", action="store_true")
parser.add_argument("--actor-correct", help="if the actor should correct weights", action="store_true")
...
log_dir = f"results/symbolic/01_algorithms/nln_ppo_tab_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
...
if __name__ == "__main__":
    ...
    for run in range(args["num_runs"]):
        ...
        actor = Actor(
            ...
            and_postprocessor=CorrectionPostprocessor() if args["actor_correct"] else NoPostprocessor(),
            negated_inputs=args["actor_negated"],
            assume_false=args["actor_assume_false"],
            device=device
        )
