...
parser.add_argument("--actor-drop-false", help="if the actor should disregard false", action="store_true")
...
log_dir = f"results/symbolic/05_initialization/nln_ppo_tab_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
...
if __name__ == "__main__":
    ...
    for run in range(args["num_runs"]):
        ...
        actor = Actor(
            ...
            drop_false=args["actor_drop_false"],
            device=device
        )
