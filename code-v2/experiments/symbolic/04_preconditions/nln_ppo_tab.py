...
from nesyrl.agents.symbolic import ActorValidity, CriticTab, ActorCriticOptimizer, PPOPolicy
...
def save_run(
    run: int, ep: int | None, actor: ActorValidity, critic: CriticTab, optim: ActorCriticOptimizer
) -> None:
...
def next_epoch(
    ep: int, run: int, actor: ActorValidity, critic: CriticTab, optim: ActorCriticOptimizer
) -> None:
    if ep % 10 == 0:
...
parser.add_argument("--actor-high", help="the high value for the actor's priors", type=float, required=True)
...
log_dir = f"results/symbolic/04_preconditions/nln_ppo_tab_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
