...
from nesyrl.util.collecting import SuccessCollector, TransitionCollector
...
log_dir = f"results/symbolic/08_effects/collect_nln_ppo_tab_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
...
if __name__ == "__main__":
    ...
    for run in range(args["num_runs"]):
        ...
        # Prepare training
        train_collector = TransitionCollector(f"{log_dir}/run_{run}_transitions.txt", policy, train_venv, ReplayBuffer(args["trainer"]["step_per_collect"]))
