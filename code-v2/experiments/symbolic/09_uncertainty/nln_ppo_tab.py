...
from nesyrl.envs.symbolic import BlocksWorldUncertain
from nesyrl.agents.symbolic import Actor, CriticTab, ActorCriticOptimizer, PPOPolicy
from nesyrl.logic.neural import ConstantInitializer, UniformInitializer, NLAndBiMixed, UncertaintyRegularizer
...
parser.add_argument("--obs-uncertainty", help="observation uncertainty", type=float, default=None)
parser.add_argument("--act-uncertainty", help="action uncertainty", type=float, default=None)
parser.add_argument("--soft-act-uncertainty", help="if an uncertain action should only left the state as was", action="store_true")

log_dir = f"results/symbolic/03_uncertainty/nln_ppo_tab_{time.strftime('%d%m%Y_%H%M%S', time.gmtime(time.time()))}"
device = "cuda" if torch.cuda.is_available() else "cpu"
torch.set_default_dtype(torch.float32)

gym.register(id="nesyrl-symbolic/BlocksWorldUncertain-v0", entry_point=BlocksWorldUncertain)
...
if __name__ == "__main__":
    ...
    for run in range(args["num_runs"]):
        ...
        train_env = gym.make("nesyrl-symbolic/BlocksWorldUncertain-v0",
                             **args["train_env_kwargs"],
                             observation_uncertainty=args["obs_uncertainty"],
                             action_uncertainty=args["act_uncertainty"],
                             soft_action_uncertainty=args["soft_act_uncertainty"])
        test_env = gym.make("nesyrl-symbolic/BlocksWorldUncertain-v0",
                            **args["test_env_kwargs"],
                            observation_uncertainty=args["obs_uncertainty"],
                            action_uncertainty=args["act_uncertainty"],
                            soft_action_uncertainty=args["soft_act_uncertainty"])
        ...
        optim = ActorCriticOptimizer(
            torch.optim.RMSprop(actor.parameters(), lr=args["actor_lr"]),
