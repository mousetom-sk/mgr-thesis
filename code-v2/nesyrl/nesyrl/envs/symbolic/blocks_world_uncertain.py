...
class BlocksWorldUncertain(SymbolicEnvironment):
    ...
    _observation_uncertainty: float
    _action_uncertainty: float
    _soft_action_uncertainty: bool
    ...
    def __init__(
        ...
        observation_uncertainty: float | None = None, action_uncertainty: float | None = None,
        soft_action_uncertainty: bool = True
    ) -> None:
        super().__init__()
        ...
        self._observation_uncertainty = observation_uncertainty
        self._action_uncertainty = action_uncertainty
        self._soft_action_uncertainty = soft_action_uncertainty
    ...
    def _perform_action(self, action: ActionAtom) -> None:
        ...
        if self._action_uncertainty is not None and self.np_random.random() < self._action_uncertainty:
            if not self._soft_action_uncertainty:
                self._current_state = self._invalid_state
            return
    ...
    def _fuzzify_observation(self, observation: Valuation) -> Valuation:
        if self._observation_uncertainty is None or self.semantics.is_true(Contradiction(), observation):
            return observation
        
        fuzzy = dict(observation)
        
        for atom in self.state_atoms:
            if isinstance(atom, Contradiction) or atom in self.domain_atoms:
                continue

            if self.semantics.is_true(atom, observation):
                fuzzy[str(atom)] -= self._observation_uncertainty * self.np_random.random()
            elif self.semantics.is_false(atom, observation):
                fuzzy[str(atom)] += self._observation_uncertainty * self.np_random.random()
        
        return fuzzy

    def _get_observation(self) -> Valuation:
        return self._fuzzify_observation(copy.deepcopy(self._current_state))
