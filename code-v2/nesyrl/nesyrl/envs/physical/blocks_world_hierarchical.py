...
class NicoBlocksWorld(NicoBlocksWorldBase):
    ...
    def _init_observation_space(self) -> None:
        size = 3 + 3 + 3 + len(self._active_joints) + 3 + 3
    ...
    def _generate_random_move(self) -> List[str]:
        ...
        for i, stack1 in enumerate(self._current_blocks_state):
            ...
            for j, stack2 in enumerate(self._current_blocks_state):
                ...
                if len(stack2) == 0:
                    valid_moves.append((block, "back", f"front{self._table}{j}", f"{self._table}{j}"))
                else:
                    valid_moves.append((block, "back", f"front{self._table}{j}", stack2[-1]))
    ...
    def _magnetize_block(self, block: str) -> None:
        ...
        i = [block in s for s in self._current_blocks_state].index(True)
        self._current_blocks_state[i].pop()
    ...
    def _demagnetize_block(self, to: str) -> None:
        ...
        if to in self._blocks:
            ...
            i = [to in s for s in self._current_blocks_state].index(True)
        else:
        ...
        self._current_blocks_state[i].append(self._magnetized_block)
        self._magnetized_block = None
    ...
    def prepare_move(self, src: str, dst: str) -> PhysicalObservation:
        if dst == "table":
            free_stacks = [i for i in range(len(self._current_blocks_state))
                           if len(self._current_blocks_state[i]) == 0]
            # i = self.np_random.choice(free_stacks)

            for i in (2, 3, 1, 4, 0, 5):
                if i in free_stacks:
                    break

            self._subgoals = [src, "back", f"front{self._table}{i}", f"{self._table}{i}", self._home]
        else:
            i = [dst in s for s in self._current_blocks_state].index(True)
            self._subgoals = [src, "back", f"front{self._table}{i}", dst, self._home]
