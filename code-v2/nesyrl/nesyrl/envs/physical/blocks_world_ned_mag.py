...
class NedBlocksWorldBase(PhysicalEnvironment):
    ...
    def _init_pybullet(self) -> None:
        ...
        self._pbc.resetDebugVisualizerCamera(cameraDistance=0.5,
                                             cameraYaw=90,
                                             cameraPitch=-25,
                                             cameraTargetPosition=[0.3, 0, 0.85])
    ...
    def _is_robot_collision(self) -> bool:
        ...
        cps = filter(lambda cp:
                     cp[2] != self._robot
                     or (not (cp[3] == 6 and cp[4] == 8)
                         and not (cp[3] == 4 and cp[4] == 6)),
                     cps)
...
class NedBlocksWorldMove(NedBlocksWorldBase):
    ...
    def _init_observation_space(self) -> None:
        size = 3 + 4 + 3 + 3 + len(self._active_joints) + 1
    ...
    def _evaluate_movement_to_closest(self) -> float:
        ...
        if min(dist_to_closest, self._last_dist_to_closest) > 2 * self._block_size:
            return 0.0
        
        delta = dist_to_closest - self._last_dist_to_closest
    ...
    def _evaluate_movement_to_table(self) -> float:
        ...
        if min(dist_to_table, last_dist_to_table) > self._block_size:
            return 0.0
        
        delta = dist_to_table - last_dist_to_table
        delta = delta if delta < 0 else delta / 2
