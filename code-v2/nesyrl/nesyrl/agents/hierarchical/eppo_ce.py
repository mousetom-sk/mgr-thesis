...
class EligibilityPPO(Agent):
    ...
    def _setup_physical(self, environment: SymbolicOptionsEnvironment):
        ...
        self._physical_extractor_optimizers = {
            key: torch.optim.Adam(p.parameters(), 1e-4) for key, p in self._physical_extractors.items()
        }

        self._physical_policy_optimizers = {
            key: torch.optim.Adam(p.parameters(), 1e-3) for key, p in self._physical_policies.items()
        }

        self._physical_vf_optimizers = {
            key: torch.optim.Adam(p.parameters(), 1e-3) for key, p in self._physical_vfs.items()
        }
    ...
    def _train_episode(self, environment: SymbolicOptionsEnvironment, subepisodes: int, n_iter: int):
        gamma = 0.99
        total_reward = 0
        I = 1
        ...
        while not done:
            ...
            if isinstance(option_atom, Grasp):
                ...
                for i in range(subepisodes):
                    reward, terminated, truncated, info = self._train_subepisode(environment, snapshot, option_atom, executor, n_iter)
    ...
    def _train_subepisode(self, environment: SymbolicOptionsEnvironment, snapshot: Any, option_atom: ActionAtom, executor: str, n_iter: int):
        gamma = 0.99
        l = 0
        clip = 0.2
        total_reward = 0
        ...
        while not done:
            ...
            action_log_prob = action_dist.log_prob(action).detach()
            ...
            for _ in range(n_iter):
                ...
                action_dist_corr = 1e-6 * torch.eye(action_dist_mean.shape[0]).to(action_dist_mean)
                action_dist = torch.distributions.MultivariateNormal(action_dist_mean, action_dist_corr)
                curr_log_prob = action_dist.log_prob(action)

                v = self._physical_vfs[executor].forward(obs_vec)

                ratio = torch.exp(curr_log_prob - action_log_prob)
                
                surr1 = ratio
                surr2 = torch.clamp(ratio, 1 - clip, 1 + clip)

                # TODO: save/restore grad
                ...
                loss = - I * torch.min(surr1, surr2) - v
                loss.backward()
    ...
    def train(self, environment: SymbolicOptionsEnvironment, episodes: int, subepisodes: int, n_iter: int) -> List[float]:
        reward_history = []
        goal_history = []
        ...
        for i in range(episodes):
            ...
            total_reward, is_goal = self._train_episode(environment, subepisodes, n_iter)
