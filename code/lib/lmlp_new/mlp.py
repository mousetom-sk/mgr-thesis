...
class LMLPLayer:
    ...
    def init_weights(self):
        if self.trainable:
            self.W = self.weights_initializer.initialize(
                self.input_dim + self.use_bias, self.output_dim
            )
        else:
            self.W = 10 * np.ones((self.output_dim, self.input_dim + self.use_bias))
...
class LMLP:
    ...
    def _add_layer(self, layer: LMLPLayer | LMLPCompositeLayer):
        if isinstance(layer, LMLPCompositeLayer):
            ...
            layer.input_dim = self.output_dim
