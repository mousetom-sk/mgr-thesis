...
class IdentityInitializer(WeightsInitializer):

    def initialize(self, input_dim: int, output_dim: int) -> nptype.NDArray:
        return np.identity(output_dim)


class OneSigmoidInitializer(WeightsInitializer):

    def initialize(self, input_dim: int, output_dim: int) -> nptype.NDArray:
        return 10 * np.ones((output_dim, input_dim))
